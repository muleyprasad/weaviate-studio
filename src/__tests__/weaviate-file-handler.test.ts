import {
  isValidWeaviateConnection,
  parseWeaviateFile,
  generateUniqueConnectionName,
} from '../utils/weaviateFileHandler';
...
describe('Weaviate File Handler', () => {
  ...
  describe('isValidWeaviateConnection', () => {
    ...
    test('should reject connections without name', () => {
      ...
      expect(isValidWeaviateConnection({ name: '', type: 'custom', httpHost: 'localhost' })).toBe(
        false
      );
      expect(
        isValidWeaviateConnection({ name: '   ', type: 'custom', httpHost: 'localhost' })
      ).toBe(false);
    });
    ...
    test('should reject connections with invalid type', () => {
      expect(
        isValidWeaviateConnection({ name: 'Test', type: 'invalid', httpHost: 'localhost' })
      ).toBe(false);
    });
    ...
    test('should reject custom connections without httpHost', () => {
      ...
      expect(isValidWeaviateConnection({ name: 'Test', type: 'custom', httpHost: '   ' })).toBe(
        false
      );
    });
    ...
    test('should reject cloud connections without cloudUrl', () => {
      ...
      expect(
        isValidWeaviateConnection({ name: 'Test', type: 'cloud', cloudUrl: '', apiKey: 'key' })
      ).toBe(false);
    });

    test('should reject cloud connections without apiKey', () => {
      expect(isValidWeaviateConnection({ name: 'Test', type: 'cloud', cloudUrl: 'url' })).toBe(
        false
      );
      expect(
        isValidWeaviateConnection({ name: 'Test', type: 'cloud', cloudUrl: 'url', apiKey: '' })
      ).toBe(false);
    });
  });
  ...
  describe('generateUniqueConnectionName', () => {
    ...
    test('should be case-insensitive', () => {
      const result = generateUniqueConnectionName('Test Connection', [
        'test connection',
        'TEST CONNECTION (2)',
      ]);

...
def create_jeopardy_collection(client):
    ...
    try:
        ...
        print(
            "✓ JeopardyQuestion collection created successfully with generative config!"
        )
        ...
        print(
            f"✓ Successfully imported {success_count} out of {len(data)} Jeopardy questions"
        )
        return True
    ...
    try:
        author_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="birthYear", data_type=wvc.config.DataType.INT
                ),
                wvc.config.Property(
                    name="isActive", data_type=wvc.config.DataType.BOOL
                ),
                wvc.config.Property(
                    name="address",
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="street", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="city", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="country", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="zipCode", data_type=wvc.config.DataType.TEXT
                        ),
                    ],
                ),
                wvc.config.Property(
                    name="coordinates", data_type=wvc.config.DataType.GEO_COORDINATES
                ),
            ],
        )
    ...
    try:
        publisher_collection = client.collections.create(
            ...
            properties=[
                wvc.config.Property(name="name", data_type=wvc.config.DataType.TEXT),
                wvc.config.Property(
                    name="foundedYear", data_type=wvc.config.DataType.INT
                ),
                wvc.config.Property(
                    name="website",
                    data_type=wvc.config.DataType.TEXT,
                ...
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="email", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="phone", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="address", data_type=wvc.config.DataType.TEXT
                        ),
                    ],
                ),
                wvc.config.Property(
                    name="headquarters", data_type=wvc.config.DataType.GEO_COORDINATES
                ),
            ],
        )
    ...
    try:
        book_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="publishedDate", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="pageCount", data_type=wvc.config.DataType.INT
                ),
                ...
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="language", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="edition", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="format", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="weight", data_type=wvc.config.DataType.NUMBER
                        ),
                    ],
                ),
            ],
            references=[
                wvc.config.ReferenceProperty(
                    name="writtenBy", target_collection="Author"
                ),
                wvc.config.ReferenceProperty(
                    name="publishedBy", target_collection="Publisher"
                ),
            ],
        )
    ...
    try:
        review_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="reviewDate", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="verified", data_type=wvc.config.DataType.BOOL
                ),
                wvc.config.Property(
                    name="reviewer",
                    data_type=wvc.config.DataType.OBJECT,
                    nested_properties=[
                        ...
                        wvc.config.Property(
                            name="memberSince", data_type=wvc.config.DataType.DATE
                        ),
                        wvc.config.Property(
                            name="totalReviews", data_type=wvc.config.DataType.INT
                        ),
                    ],
                ),
            ],
            references=[
                wvc.config.ReferenceProperty(
                    name="reviewsBook", target_collection="Book"
                ),
            ],
        )
    ...
    for i, book_data in enumerate(books_data):
        try:
            result = book_collection.data.insert(
                ...
                references={
                    "writtenBy": author_ids[i]
                    if i < len(author_ids)
                    else author_ids[0],
                    "publishedBy": publisher_ids[i]
                    if i < len(publisher_ids)
                    else publisher_ids[0],
                },
            )
    ...
    try:
        github_user_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="location", data_type=wvc.config.DataType.TEXT
                ),
                wvc.config.Property(
                    name="email",
                    data_type=wvc.config.DataType.TEXT,
                ...
                wvc.config.Property(
                    name="hireable", data_type=wvc.config.DataType.BOOL
                ),
                wvc.config.Property(
                    name="createdAt", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="stats",
                    data_type=wvc.config.DataType.OBJECT,
                    nested_properties=[
                        wvc.config.Property(
                            name="publicRepos", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="publicGists", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="followers", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="following", data_type=wvc.config.DataType.INT
                        ),
                    ],
                ),
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="htmlUrl", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="blog", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="twitterUsername", data_type=wvc.config.DataType.TEXT
                        ),
                    ],
                ),
            ],
    ...
    try:
        github_repo_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="archived", data_type=wvc.config.DataType.BOOL
                ),
                wvc.config.Property(
                    name="createdAt", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="updatedAt", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="pushedAt", data_type=wvc.config.DataType.DATE
                ),
                ...
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="stargazersCount", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="watchersCount", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="forksCount", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="openIssuesCount", data_type=wvc.config.DataType.INT
                        ),
                    ],
                ),
                wvc.config.Property(
                ...
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="key", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="name", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="spdxId", data_type=wvc.config.DataType.TEXT
                        ),
                    ],
                ),
            ],
            references=[
                wvc.config.ReferenceProperty(
                    name="ownedBy", target_collection="GitHubUser"
                ),
            ],
        )
    ...
    try:
        github_issue_collection = client.collections.create(
            ...
            properties=[
                ...
                wvc.config.Property(
                    name="closedAt", data_type=wvc.config.DataType.DATE
                ),
                wvc.config.Property(
                    name="labels",
                    data_type=wvc.config.DataType.OBJECT,
                    nested_properties=[
                        wvc.config.Property(
                            name="names", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="colors", data_type=wvc.config.DataType.TEXT
                        ),
                        wvc.config.Property(
                            name="count", data_type=wvc.config.DataType.INT
                        ),
                    ],
                ),
                wvc.config.Property(
                    ...
                    nested_properties=[
                        wvc.config.Property(
                            name="totalCount", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="plusOne", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="minusOne", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="laugh", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="hooray", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="confused", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="heart", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="rocket", data_type=wvc.config.DataType.INT
                        ),
                        wvc.config.Property(
                            name="eyes", data_type=wvc.config.DataType.INT
                        ),
                    ],
                ),
            ],
            references=[
                wvc.config.ReferenceProperty(
                    name="belongsToRepo", target_collection="GitHubRepo"
                ),
                wvc.config.ReferenceProperty(
                    name="createdBy", target_collection="GitHubUser"
                ),
            ],
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                user_obj = {
                    ...
                    if user_data.get("hireable") is not None
                    else False,
                    ...
                    for repo_data in repos_data[:2]:  # Limit to 2 repos per user
                        try:
                            repo_obj = {
                                ...
                                "metrics": {
                                    "stargazersCount": repo_data.get(
                                        "stargazers_count", 0
                                    ),
                                    "watchersCount": repo_data.get("watchers_count", 0),
                                    "forksCount": repo_data.get("forks_count", 0),
                                    "openIssuesCount": repo_data.get(
                                        "open_issues_count", 0
                                    ),
                                },
                                "topics": ", ".join(repo_data.get("topics", [])),
                                "license": {
                                    "key": repo_data.get("license", {}).get("key", "")
                                    if repo_data.get("license")
                                    else "",
                                    "name": repo_data.get("license", {}).get("name", "")
                                    if repo_data.get("license")
                                    else "",
                                    "spdxId": repo_data.get("license", {}).get(
                                        "spdx_id", ""
                                    )
                                    if repo_data.get("license")
                                    else "",
                                ...
                                if repo_data.get("license")
                                else {"key": "", "name": "", "spdxId": ""},
                            }

                            repo_result = github_repo_collection.data.insert(
                                properties=repo_obj, references={"ownedBy": result}
                            )
                            repo_ids.append(repo_result)
                            print(
                                f"  ✓ Inserted repo: {repo_data.get('name', 'Unknown')}"
                            )
                                ...
                                for issue_data in issues_data[
                                    ...
                                    try:
                                        ...
                                        issue_obj = {
                                            ...
                                            "createdAt": issue_data.get(
                                                "created_at", ""
                                            ),
                                            "updatedAt": issue_data.get(
                                                "updated_at", ""
                                            ),
                                            ...
                                            "reactions": {
                                                "totalCount": issue_data.get(
                                                    "reactions", {}
                                                ).get("total_count", 0),
                                                "plusOne": issue_data.get(
                                                    "reactions", {}
                                                ).get("+1", 0),
                                                "minusOne": issue_data.get(
                                                    "reactions", {}
                                                ).get("-1", 0),
                                                "laugh": issue_data.get(
                                                    "reactions", {}
                                                ).get("laugh", 0),
                                                "hooray": issue_data.get(
                                                    "reactions", {}
                                                ).get("hooray", 0),
                                                "confused": issue_data.get(
                                                    "reactions", {}
                                                ).get("confused", 0),
                                                "heart": issue_data.get(
                                                    "reactions", {}
                                                ).get("heart", 0),
                                                "rocket": issue_data.get(
                                                    "reactions", {}
                                                ).get("rocket", 0),
                                                "eyes": issue_data.get(
                                                    "reactions", {}
                                        ...
                                        print(
                                            f"    ✓ Inserted issue: {issue_data.get('title', 'Unknown')[:50]}..."
                                        )
                            ...
                            print(
                                f"  ✗ Failed to insert repo {repo_data.get('name', 'Unknown')}: {e}"
                            )
...
def verify_collections(client):
    ...
    try:
        ...
        # Check which collections support RAG
        rag_capable = []
        for col in collections:
            has_generative = bool(
                col.generative and col.generative.name and col.generative.name != "none"
            )
            rag_marker = " [RAG]" if has_generative else ""
            print(f"  • {col.name}{rag_marker}")
            if has_generative:
                rag_capable.append(col.name)

        if rag_capable:
            print(f"\n✓ RAG-capable collections: {', '.join(rag_capable)}")
        else:
            print("\n⚠ Warning: No collections with generative config found")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if (
                        ...
                        and "Author" in result["data"]["Get"]
                    ):
                        authors = result["data"]["Get"]["Author"]
                        if authors and len(authors) > 0:
                            author = authors[0]
                            print(
                                f"✓ Nested properties test passed - Author: {author.get('name', 'Unknown')}"
                            )
                            if "address" in author:
                                print(
                                    f"  - City: {author['address'].get('city', 'Unknown')}"
                                )
                            if "coordinates" in author:
                                print(f"  - Coordinates: {author['coordinates']}")
                        else:
                            print(
                                "⚠ Warning: No authors returned in nested properties test"
                            )
                    else:
                        print(
                            "⚠ Warning: Unexpected response structure in nested properties test"
                        )
                else:
                    print(
                        f"⚠ Warning: Nested properties test failed with status: {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if (
                        "data" in result
                        and "Get" in result["data"]
                        and "Book" in result["data"]["Get"]
                    ):
                        books = result["data"]["Get"]["Book"]
                        if books and len(books) > 0:
                            book = books[0]
                            print(
                                f"✓ Cross-reference test passed - Book: {book.get('title', 'Unknown')}"
                            )
                            if "writtenBy" in book and book["writtenBy"]:
                                print(
                                    f"  - Author: {book['writtenBy'].get('name', 'Unknown')}"
                                )
                            if "publishedBy" in book and book["publishedBy"]:
                                print(
                                    f"  - Publisher: {book['publishedBy'].get('name', 'Unknown')}"
                                )
                        else:
                            print(
                                "⚠ Warning: No books returned in cross-reference test"
                            )
                    else:
                        print(
                            "⚠ Warning: Unexpected response structure in cross-reference test"
                        )
                else:
                    print(
                        f"⚠ Warning: Cross-reference test failed with status: {response.status_code}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Populate Weaviate with comprehensive test data"
    )
    parser.add_argument(
        "--skip-github", action="store_true", help="Skip GitHub data fetching"
    )
    parser.add_argument(
    ...
    if args.verify_only:
        ...
        print(
            "  • Various data types (text, numbers, booleans, dates, geo coordinates)"
        )
